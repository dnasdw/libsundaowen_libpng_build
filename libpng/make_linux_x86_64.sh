...
cd "$rootdir/project"
cmake -DBUILD64=ON -C "$rootdir/CMakeLists.txt" -DPNG_SHARED=OFF -DPNG_TESTS=OFF -DSKIP_INSTALL_FILES=ON -DSKIP_INSTALL_EXPORT=ON -DZLIB_INCLUDE_DIR="$rootdir/../zlib/include/$target" -DZLIB_LIBRARY="$rootdir/../zlib/lib/$target/libz.a" -DM_LIBRARY="${M_LIBRARY};/usr/lib32" -DCMAKE_INSTALL_PREFIX="$prefix" "$rootdir/$version"
